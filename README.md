...
- optional Runpod Flash deployment through [flash_worker.py](flash_worker.py)
...
- [src/pipeline/run_lira.py](src/pipeline/run_lira.py): main experiment entrypoint
- [src/pipeline/train_target.py](src/pipeline/train_target.py): target-model training and evaluation
- [src/attacks/lira.py](src/attacks/lira.py): LiRA scoring utilities
- [src/data/eld.py](src/data/eld.py): ELD loading and preprocessing
- [flash_worker.py](flash_worker.py): Runpod Flash endpoint for remote execution
- [agents.md](agents.md): repo-specific implementation notes and goals
...
If you are starting from a new GPU machine and want the safest path with the least wasted paid time, use [bootstrap_and_run.sh](bootstrap_and_run.sh).
...
- results should be discussed as a credible partial reproduction, not as the original paper implementation
...
These presets are implemented in [run_lira.py](src/pipeline/run_lira.py) and are intended to reduce command-line drift when comparing runs.
...
This repo includes a Flash endpoint in [flash_worker.py](flash_worker.py).
...
- building a runnable baseline
- validating that the attack pipeline works end to end
- comparing local changes against a stable reproduction-oriented starting point

This repo should not be described as:

- the original paper codebase
- a complete implementation of every experiment from the paper

For paper-facing discussion, the safest description is:

"A partial implementation and reproduction-oriented baseline for record-level LiRA in time-series forecasting, aligned to the cited paper with documented dataset-handling deviations where public mirrors do not exactly match the original preprocessing outcome."
